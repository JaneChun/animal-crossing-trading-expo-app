...
2. **커밋은 사용자가 직접 수행** — 변경은 논리적 커밋 단위로 하나씩 진행하고, 각 단위 완료 시 검토 요청과 추천 커밋 메시지를 제공한 뒤 사용자 확인 후 다음 단위로 진행. 자동 커밋 금지. PR/커밋에 AI 출처 표기 금지.
...
9. **복합 데이터 변환은 의미 있는 중간 변수로 단계 표현** — 중첩 호출보다 각 값의 도메인 의미와 데이터 흐름이 이름에서 드러나게 작성.
