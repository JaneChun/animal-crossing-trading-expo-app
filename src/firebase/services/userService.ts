...
export const getUserInfo = async (uid: string): Promise<UserInfo | null> => {
	return firestoreRequest('나의 정보 조회', async () => {
		...
		return { uid: id, ...userData } as UserInfo;
	});
};
...
export const saveUserInfo = async (
	...
	await firestoreRequest('나의 정보 저장', async () => {
		await setDoc(doc(db, 'Users', uid), data, { merge: true });
	});
};

export const getPublicUserInfo = async (creatorId: string): Promise<PublicUserInfo> => {
	const result = await firestoreRequest('유저 정보 조회', async () => {
		const docData = await getDocFromFirestore({
			collection: 'Users',
			id: creatorId,
		});
	...
	return result ?? getDefaultUserInfo(creatorId);
};
...
export const getPublicUserInfos = async (
	...
	const result = await firestoreRequest('유저 정보 일괄 조회', async () => {
...
export const savePushTokenToFirestore = async ({
	...
	await firestoreRequest('유저 푸시 토큰 저장', async () => {
